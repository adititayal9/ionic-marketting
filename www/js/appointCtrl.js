 myapp.controller("appointCtrl", function($scope, $state, $cordovaGeolocation, $http, $ionicPopup) {

     $scope.activityId = $state.params.activityId;
     //  console.log("hi "+$scope.activityId );
     $scope.toggle = localStorage.getItem("appoint_toggle");
    ...
    var hasAppointSummDetails = checkLocalStorage('appointsummaryDetails');
    console.log(hasAppointSummDetails);
    if(hasAppointSummDetails) {
        $scope.appointsummaryDetails=JSON.parse(localStorage.getItem("appointsummaryDetails") || {});
        console.log($scope.appointsummaryDetails);
    }
     ...
     $scope.change = function() {
         console.log($scope.appointsummaryDetails);
         localStorage.setItem('appointsummaryDetails', JSON.stringify($scope.appointsummaryDetails));
    ...
    $scope.save = function() {
        ...
        $http({
            ...
            params: {
                ...
                activitySummary: JSON.stringify($scope.appointsummaryDetails)
            }
    ...
    $scope.start_stop = function() {
        ...
        if ($scope.toggle == "Start Activity") {
            ...
            $http({
                ...
                if (response.data.Message == "Other Activity Present") {
                    ...
                    localStorage.setItem("appoint_toggle", "End activity"); 
            ...
            $http({
                ...
                if(response.data.StatusCode==200)
                {
                   $ionicPopup.alert({
                    ...
                    localStorage.setItem("appoint_toggle", "Completed !!"); 
     ...
     $scope.getLocation = function() {
         ...
         $cordovaGeolocation
             .getCurrentPosition(posOptions)
             .then(function(position) {
                $scope.appointsummaryDetails.lat = position.coords.latitude;
                $scope.appointsummaryDetails.lng = position.coords.longitude;
                localStorage.setItem('appointsummaryDetails', JSON.stringify($scope.appointsummaryDetails));
