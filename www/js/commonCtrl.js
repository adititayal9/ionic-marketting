 myapp.controller("commonCtrl", function($scope, $state, $cordovaGeolocation, $http, $ionicPopup) {
     ...
     $scope.commonsummaryDetails=
    {
        "comment": ""
    };
 

    var hascommonSummDetails = checkLocalStorage('commonsummaryDetails');
    console.log(hascommonSummDetails);
    if(hascommonSummDetails) {
        $scope.commonsummaryDetails=JSON.parse(localStorage.getItem("commonsummaryDetails") || {});
        console.log($scope.commonsummaryDetails);
    }
     ...
     $scope.change = function() {
         console.log($scope.commonsummaryDetails);
         localStorage.setItem('commonsummaryDetails', JSON.stringify($scope.commonsummaryDetails));
    ...
    $scope.save = function() {
        ...
        $http({
            ...
            params: {
                ...
                activitySummary: JSON.stringify($scope.commonsummaryDetails)
            }
    ...
    $scope.start_stop = function() {
        ...
        if ($scope.toggle == "Start Activity") {
            ...
            $http({
                ...
                if (response.data.Message == "Other Activity Present") {
                    ...
                    localStorage.setItem("common_toggle", "End activity"); 
            ...
            $http({
                ...
                if(response.data.StatusCode==200)
                {
                   $ionicPopup.alert({
                    ...
                    localStorage.setItem("common_toggle", "Completed !!"); 
     ...
     $scope.getLocation = function() {
         ...
         $cordovaGeolocation
             .getCurrentPosition(posOptions)
             .then(function(position) {
                $scope.commonsummaryDetails.lat = position.coords.latitude;
                $scope.commonsummaryDetails.lng = position.coords.longitude;
                localStorage.setItem('commonsummaryDetails', JSON.stringify($scope.commonsummaryDetails));
