 myapp.controller("online_researchCtrl", function($scope, $state, $cordovaGeolocation, $http, $ionicPopup) {

     $scope.activityId = $state.params.activityId;
     //  console.log("hi "+$scope.activityId );
     $scope.toggle = localStorage.getItem("online_research_toggle");
     ...
     $scope.online_researchsummaryDetails = {
         "comment": "research purpose"
     };

    var hasonline_researchSummDetails = checkLocalStorage('online_researchsummaryDetails');
    console.log(hasonline_researchSummDetails);
    if(hasonline_researchSummDetails) {
        $scope.online_researchsummaryDetails=JSON.parse(localStorage.getItem("online_researchsummaryDetails") || {});
        console.log($scope.online_researchsummaryDetails);
    }
     ...
     $scope.change = function() {
         console.log($scope.online_researchsummaryDetails);
         localStorage.setItem('online_researchsummaryDetails', JSON.stringify($scope.online_researchsummaryDetails));
    ...
    $scope.save = function() {
        ...
        $http({
            ...
            params: {
                ...
                activitySummary: JSON.stringify($scope.online_researchsummaryDetails)
            }
    ...
    $scope.start_stop = function() {
        ...
        if ($scope.toggle == "Start Activity") {
            ...
            $http({
                ...
                if (response.data.Message == "Other Activity Present") {
                    ...
                    localStorage.setItem("online_research_toggle", "End activity"); 
            ...
            $http({
                ...
                if(response.data.StatusCode==200)
                {
                   $ionicPopup.alert({
                    ...
                    localStorage.setItem("online_research_toggle", "Completed !!"); 
     ...
     $scope.getLocation = function() {
         ...
         $cordovaGeolocation
             .getCurrentPosition(posOptions)
             .then(function(position) {
                $scope.online_researchsummaryDetails.lat = position.coords.latitude;
                $scope.online_researchsummaryDetails.lng = position.coords.longitude;
                localStorage.setItem('online_researchsummaryDetails', JSON.stringify($scope.online_researchsummaryDetails));
